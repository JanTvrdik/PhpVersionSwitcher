...
namespace PhpVersionSwitcher
{
	internal static class Symlinks
	{
		...
		internal static class NativeMethods
		{
			...
			internal static extern SafeFileHandle CreateFile(string lpFileName, int dwDesiredAccess, int dwShareMode, IntPtr securityAttributes, int dwCreationDisposition, int dwFlagsAndAttributes, IntPtr hTemplateFile);
		...
		{
			SafeFileHandle fileHandle = NativeMethods.CreateFile(symlink, 0, 2, IntPtr.Zero, CreationDispositionOpenExisting, FileFlagBackupSemantics, IntPtr.Zero);
		...
		{
			return NativeMethods.CreateSymbolicLink(symlink, target, SymbolicLinkFlagDirectory);
		}
	}
}
